 ...
 *       本 Java 类专注面试题核心逻辑剖析、前缀区分度计算公式推演与三星索引优化策略讲解。
 * 
 * 核心考点深度解析：
 * 1. 索引是不是建的越多越好？
 *    - 绝不是！单表建议不超过 5 个。
 ...
 *    - 特例场景：极端数据倾斜 (如 1000 万条中仅 10 条 status=0 待处理积压订单)，或作为联合索引辅助列。
 * 3. 前缀索引 (Prefix Index)：
...
public class Index06_PrefixIndexAndOptimizationDemo {
    ...
    public static void runDemo() {
        ...
        // 1. 实测前缀区分度计算 (Selectivity 计算公式)
        String selectivitySql = "SELECT "
                + "  COUNT(DISTINCT LEFT(buyer_email, 5)) / COUNT(*) AS sel_len_5, "
                + "  COUNT(DISTINCT LEFT(buyer_email, 8)) / COUNT(*) AS sel_len_8, "
                + "  COUNT(DISTINCT buyer_email) / COUNT(*) AS sel_full "
                + "FROM interview_orders;";
        DbConnectionHelper.printQueryResults("前缀索引不同截取长度的区分度对比计算", selectivitySql);

        // 2. 验证前缀索引执行计划 (能够走索引 ref)
        DbConnectionHelper.printQueryResults("前缀索引执行计划验证 (type=ref)", 
                "EXPLAIN SELECT * FROM interview_orders WHERE buyer_email = 'zhangsan_dev@company.com';");

        // 3. 验证前缀索引无法实现覆盖索引 (即便只查 buyer_email 依然无法 Using index，必须回表！)
        DbConnectionHelper.printQueryResults("前缀索引无法作为覆盖索引验证 (Extra 无法出现 Using index)", 
                "EXPLAIN SELECT buyer_email FROM interview_orders WHERE buyer_email = 'zhangsan_dev@company.com';");

        // 4. 数据倾斜状态字段查询 (status=0 占比极小，走联合索引 idx_status_created)
        DbConnectionHelper.printQueryResults("极端数据倾斜状态值查询 (status=0 走 idx_status_created 索引)", 
                "EXPLAIN SELECT * FROM interview_orders WHERE order_status = 0;");
